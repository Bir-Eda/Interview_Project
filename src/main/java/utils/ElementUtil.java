...
public class ElementUtil {
     ...
     * @return
     */
    public int findTheNumberOfFrame(By locator) {
        int i = 0;
        int frameCount = BasePage.get().findElements(By.tagName("iframe")).size();
        System.out.println("Frame count : " + frameCount);
        for (i = 0; i < frameCount; i++) {
            BasePage.get().switchTo().frame(i);
            int count = BasePage.get().findElements(locator).size();
            if (count > 0) {
     ...
     * @param value
     */
    public void singleValueSelectionDropDown( By locator, String value) {
        List<WebElement> droplist = BasePage.get().findElements(locator);
